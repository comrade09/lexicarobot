...
async def save_video_code(question_code: str, message_id: int) -> None:
...
async def get_video_message_id(question_code: str) -> int | None:
    data = video_collection.find_one({"_id": question_code.upper()})
    return data.get("message_id") if data else None


async def increment_view_count(question_code: str) -> None:
...
async def get_top_videos(limit: int = 10) -> list[dict[str, Any]]:
...
async def get_recent_codes(limit: int = 5) -> list[str]:
